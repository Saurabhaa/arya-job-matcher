# Arya Job Matcher — Build Plan (v3, OpenAI-only)

Build spec for Claude Code, structured for **phased execution** so it never runs out of context mid-build.

> **Changes from v2:** swapped Anthropic Claude → OpenAI for the reasoning layer. Embeddings + reasoning both run on OpenAI now. Locked contracts (schemas, system prompt, SSE event shape) are unchanged.
...
Do not paste multiple phase prompts at once. Do not skip ahead. The DoD gates exist so we catch drift early.

---

## TL;DR

User uploads (or pastes) a resume. App returns the top 5 matching jobs with personalized reasoning, streamed progressively. Two-stage retrieval (pgvector shortlist → OpenAI reasoning), Redis caching on two layers, Sentry observability, full local run via `docker-compose up`.
...
| LLM choice (OpenAI), justified | Phase 5 README |
...
| (Bonus) Deploy to Render/Railway/Fly | Cut by default |
...
| Cache | Redis 7 |
| LLM (reasoning) | OpenAI `gpt-5.4-mini` (current cost-efficient model with strict structured-output support) |
| Embeddings | OpenAI `text-embedding-3-small` (1536-dim) |
| Observability | `sentry-sdk[fastapi]` with one custom transaction |
| Frontend | Vite + React 18 + TypeScript + Tailwind |
| Orchestration | docker-compose |

**Do not introduce:** LangChain, LangGraph, FAISS, WebSockets, Alembic, auth, user model, test coverage targets, CI pipelines. **Do not add the `anthropic` SDK** — single-vendor is part of the design.
...
│   │       ├── llm.py          # OpenAI reasoning wrapper + MATCHER_SYSTEM_PROMPT
...
**Note for Phase 1:** if your `.env.example` from earlier still has `ANTHROPIC_API_KEY` and `CLAUDE_MODEL`, remove them as part of the Phase 3 handoff — they're harmless until then.

---

## Locked Contracts (unchanged from v2 — write first, do not change mid-build)
...
The OpenAI client enforces the JSON shape via `response_format={"type": "json_schema", "json_schema": {"name": "job_match", "strict": true, "schema": <pydantic-derived schema>}}`. Use the SDK's `client.chat.completions.parse(..., response_format=JobMatch)` helper if convenient — it auto-derives the schema from the Pydantic model and returns a parsed instance.
...
- [ ] Backend `pyproject.toml` pins: `fastapi`, `uvicorn[standard]`, `sqlalchemy[asyncio]`, `asyncpg`, `pgvector`, `redis>=5`, `openai>=1.50`, `pypdf`, `sentry-sdk[fastapi]`, `pydantic-settings`, `python-multipart`, `sse-starlette`. **Do not include `anthropic`.**
...
**Goal:** Full pipeline. 10 OpenAI calls run in parallel, each result streams as it arrives, top 5 settle in a final `done` event. Re-runs are instant.

**Scope:** OpenAI reasoning wrapper with strict structured outputs, parallel orchestration with `asyncio.as_completed`, result cache layer, token accounting.

### Pre-flight cleanup (do this first in this phase)

- [ ] Remove `ANTHROPIC_API_KEY` and `CLAUDE_MODEL` from `.env.example` and any local `.env` that may have them.
- [ ] Ensure `OPENAI_REASONING_MODEL=gpt-5.4-mini` is present in `.env.example`.
- [ ] Remove `anthropic` from `pyproject.toml` if it was added in Phase 1. Reinstall in container if it was: `docker compose build backend`.
- [ ] Update `config.py` to expose `openai_reasoning_model` (no Anthropic settings).

### Tasks

- [ ] `services/llm.py::match_job(resume_text, job)`:
  - Use the OpenAI Python SDK (`openai>=1.50`).
  - Call: `client.chat.completions.parse(model=settings.openai_reasoning_model, messages=[...], response_format=JobMatch, temperature=0)`.
  - System message: the locked `MATCHER_SYSTEM_PROMPT`.
  - User message: a structured block containing `RESUME:\n{resume_text}\n\nJOB_DESCRIPTION:\n{job.title} at {job.company}\n{job.description}`.
  - The `.parse(...)` helper returns a parsed `JobMatch` instance — no manual JSON parsing.
  - Handle refusals: if `completion.choices[0].message.refusal` is set, raise a typed error so the orchestrator can emit an `error` event for that job.
  - Extract token usage from `completion.usage` (`prompt_tokens`, `completion_tokens`) and return alongside the `JobMatch`.
...
- [ ] Cache-wrap `match_job`: check cache → return on hit → call OpenAI on miss → write back.
...
- [ ] If a single OpenAI call fails or refuses: log to Sentry, emit `{"type": "error", "data": {"job_id": N, "message": "..."}}` for that job, continue the rest. Do not break the stream.
...
- [ ] Force an error (e.g. swap to an invalid model name on one call path) — the other 9 still succeed, one `error` event surfaces.

### Handoff prompt for Claude Code

> Read `BUILD_PLAN.md` in full and inspect the current repo state. Phases 1 and 2 are complete. Execute **Phase 3 only** — including the Pre-flight cleanup at the top. Stop after every DoD checkbox in Phase 3 passes. Do not start Phase 4. For the verbatim-bullet check, include the resume text and three `highlight_bullet` values in your report so I can eyeball them. Commit with `Phase 3: OpenAI reasoning + result cache`.
...
- [ ] Set transaction tags at finish: `shortlist_size`, `cache_hits`, `cache_misses`, `total_tokens_in`, `total_tokens_out`, `estimated_usd` (compute from gpt-5.4-mini's per-token prices), `resume_hash_prefix` (first 8 chars only), `reasoning_model` (the model id from config).
...
- [ ] README sections (exactly per the brief):
  ...
  **Setup:** `git clone …` → `cp .env.example .env` → fill 2 API keys (OpenAI + Sentry) → `docker compose up -d` → seed command → open `localhost:5173`. Show real commands, not prose.
  ...
  **Architecture:** treat this as a design doc — the brief weights it heaviest. Cover:
  ...
  - **Why OpenAI (single vendor for both stages):** one SDK, one credential, one billing surface. Structured outputs via `response_format={"type":"json_schema","strict":true}` guarantee a typed `JobMatch` at decode time — no parser, no retries on malformed JSON. `gpt-5.4-mini` gives strong reasoning per dollar; for 10 parallel calls per match flow, mini is the right size. The matcher is provider-agnostic at the `services/llm.py` boundary — swapping to Anthropic or Gemini is a one-file change.
  ...
  **What I'd do differently with one more week:**
  ...
  - Switch to GPT-5.5 for the reasoning stage when quality matters more than speed/cost; benchmark side-by-side.
  - Stream OpenAI's token output to give in-token reasoning preview.
  - Rate limiting + retry/backoff on OpenAI 429s.
  - Replace pypdf with a layout-aware parser (Unstructured.io, LlamaParse) for tabular resumes.
  - Move to a larger embedding model (`text-embedding-3-large`) if shortlist precision matters more than cost.

  **Known issues / limitations / chose-not-to-fix:**
  - pypdf can't handle scanned/image-based PDFs.
  - No rate limiting on `/api/match`.
  - No retry/backoff on upstream OpenAI errors — they surface as `error` events.
...
- [ ] Fresh-clone smoke test: `cp -r` the repo to `/tmp/smoke`, follow only the README, verify the demo works.
...
8. **No premature optimization.** No connection pool tuning, no IVF index, no OpenAI response token streaming.
...
- Streaming OpenAI's token output (in-token reasoning preview)
- Rate limiting, retry/backoff on upstream APIs
- Anthropic / Gemini SDK alongside OpenAI (single-vendor is the design)
